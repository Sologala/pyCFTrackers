...
from pyCFTrackers.lib.pysot.datasets import VOTDataset,OTBDataset
from pyCFTrackers.lib.pysot.evaluation import AccuracyRobustnessBenchmark, EAOBenchmark,OPEBenchmark
from pyCFTrackers.lib.pysot.visualization import draw_success_precision, draw_eao, draw_f1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_dir', type=str, default='test/VOT2018',help='tracker result root')
    ...
    root = join(realpath(dirname(__file__)), '../dataset/VOT2018')
    tracker_dir = args.result_dir
    trackers = glob.glob(join(tracker_dir, args.tracker_prefix+'*'))
    trackers = [t.split('/')[-1] for t in trackers]
    trackers=['MCCTH-Staple','STRCF','CSRDCF-LP','DSST-LP','LDES','SAMF','OPENCV-CSRDCF','DCF','CSK','Staple','DSST','DAT','MOSSE','ECO-HC','ECO','CN','KCF','BACF','CSRDCF']
